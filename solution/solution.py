...
def batch_compare(prompts: list[str]) -> list[dict]:
    ...
    """
    return [{"prompt": p, **compare_models(p)} for p in prompts]
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    """
    def _trim(text: str, n: int = 40) -> str:
        return text if len(text) <= n else text[: n - 3] + "..."

    header = (
        f"{'Prompt':<40} | {'GPT-4o Response':<40} | {'Mini Response':<40} | "
        f"{'GPT-4o Lat':>10} | {'Mini Lat':>10}"
    )
    ...
    for r in results:
        lines.append(
            ...
            f"{r['gpt4o_latency']:>10.3f} | "
            f"{r['mini_latency']:>10.3f}"
        )
